...
const TYPE_BADGE: Record<string, string> = {
  "업무":     "bg-muted text-muted-foreground",
...
export default async function VehicleTripsPage({ searchParams }: Props) {
  ...
  /* 모든 상태의 기록 조회 (완료·작성중·제출됨·승인됨·반려됨) */
  const { data: trips } = (selectedVehicleId && driver)
    ? await supabase
        .from("trip_logs")
        .select(`id, departure_time, arrival_time, departure_location, arrival_location,
    ...
    <div className="pb-6">
      ...
      <div className="mx-4 mb-2 rounded-xl border border-border bg-background overflow-hidden">
        <div className="px-4 py-2 bg-muted/50 flex items-center gap-2">
          ...
          <span className="text-xs font-semibold text-emerald-700">🏠 출퇴근</span>
          <span className="text-xs text-emerald-600">{comTrips.length}건</span>
          <span className="mx-2 text-border">|</span>
          <span className="text-xs font-semibold text-orange-700">👤 개인</span>
          <span className="text-xs text-orange-600">{perTrips.length}건</span>
        </div>
            ...
            return (
              <div key={trip.id} className="rounded-2xl bg-background border border-border p-4 space-y-2">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className={`text-xs font-medium rounded-full px-2.5 py-0.5 ${isOngoing ? "bg-blue-100 text-blue-700" : statusInfo.cls}`}>
