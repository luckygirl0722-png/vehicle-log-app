...
export default function ExcelUploadSection({ vehicleId }: Props) {
    ...
    reader.onload = (ev) => {
      try {
        ...
        for (let i = 12; i <= 42; i++) {
          ...
          const dateVal = row[3];
          const depKm   = toNum(row[8]);
          const arrKm   = toNum(row[10]);
          const comKm   = toNum(row[14]);
          const bizKm   = toNum(row[16]);
          const perKm   = toNum(row[18]);
          const tollHp  = toNum(row[20]);
          const tollEtc = toNum(row[21]);
          ...
          parsed.push({ rowNum:i-11, date:dateStr, dep_km:depKm, arr_km:arrKm, distance:arrKm-depKm, trip_type, toll_fee:tollHp+tollEtc, note:noteVal, error:errors.length?errors.join(", "):undefined });
        }
        if (parsed.length === 0) {
          const s = aoa[12];
          setDebugInfo(`총행수:${aoa.length} / 13행샘플: D=${s?.[3]} I=${s?.[8]} K=${s?.[10]}`);
        }
  ...
  async function handleUpload() {
    ...
    const payload = validRows.map(r => ({
      departure_time: r.date, arrival_time: r.date,
      departure_km: r.dep_km, arrival_km: r.arr_km,
      toll_fee: r.toll_fee, trip_type: r.trip_type,
      note: r.note || undefined,
    }));
    const res  = await fetch("/api/trips/bulk", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ vehicle_id:vehicleId, rows:payload }) });
    const data = await res.json();
    setLoading(false);
    if (!res.ok) { setApiError(data.details?.join("\n") ?? data.error ?? "업로드 실패"); return; }
    ...
    <div className="hidden md:block mx-4 mb-4">
      <div className="rounded-2xl border border-border bg-background overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-border bg-muted/30">
          ...
          <a href="/차량운행기록부_양식.xlsx" download className="flex items-center gap-1.5 rounded-lg border border-border bg-background px-3 py-2 text-xs font-medium hover:bg-muted transition-colors shrink-0">
            📥 양식 다운로드
          </a>
        </div>
        ...
        <div className="px-5 py-4 space-y-4">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    ...
                    {invalidRows.length > 0 && <span className="text-amber-600 font-medium"> · 제외 {invalidRows.length}건</span>}
                  </p>
                </div>
                <button onClick={handleReset} className="text-xs text-muted-foreground hover:text-foreground border border-border rounded-lg px-3 py-1.5">다시 선택</button>
              </div>
              ...
              {rows.length === 0 && (
                <div className="rounded-xl bg-amber-50 border border-amber-200 px-4 py-3 space-y-1.5">
                  ...
                  {debugInfo && <p className="text-xs font-mono bg-amber-100 text-amber-600 px-2 py-1 rounded mt-1">진단: {debugInfo}</p>}
                </div>
              )}
              ...
              {rows.length > 0 && (
                <div className="rounded-xl bg-blue-50 border border-blue-200 px-4 py-2.5 text-xs text-blue-700 space-y-0.5">
                  ...
                  <p>출발지·도착지·시간은 <span className="font-semibold">"미입력"</span>으로 저장됩니다. 필요 시 앱에서 수정하세요.</p>
              ...
              {invalidRows.length > 0 && (
                <div className="rounded-xl bg-amber-50 border border-amber-200 px-4 py-3 space-y-1">
                  ...
                  {invalidRows.length > 5 && <p className="text-xs text-amber-600">외 {invalidRows.length-5}건</p>}
                </div>
              )}
                      ...
                      {validRows.map(r => (
                        <tr key={r.rowNum} className="hover:bg-muted/30">
                          ...
                          <td className="px-3 py-2">
                            <span className={`rounded-full px-2 py-0.5 font-medium ${TYPE_COLOR[r.trip_type] ?? "bg-gray-100"}`}>{r.trip_type}</span>
                          </td>
                          <td className="px-3 py-2 whitespace-nowrap text-right">{r.toll_fee > 0 ? r.toll_fee.toLocaleString("ko-KR") : "—"}</td>
              ...
              {validRows.length > 0 && (
                <div className="flex gap-3">
                  <button onClick={handleReset} className="flex-1 rounded-xl border border-border py-3 text-sm font-medium bg-background">취소</button>
                  <button onClick={handleUpload} disabled={loading} className="flex-1 rounded-xl bg-primary text-primary-foreground py-3 text-sm font-semibold disabled:opacity-50">
          ...
          {step === "done" && (
            <div className="text-center py-6 space-y-3">
              ...
              <button onClick={handleReset} className="text-xs text-muted-foreground hover:text-foreground border border-border rounded-lg px-4 py-2">다시 업로드</button>
            </div>
          )}
        </div>
      </div>
