 ...
 *
 * JSZip 은 최상단 import 없이 런타임에 require() 로 로드합니다.
 * (serverExternalPackages: ["jszip"] 설정으로 webpack 번들링 제외)
 * jszip 이 없어도 다운로드는 정상 동작하며, 있으면 서식(테두리·배경색)까지 복원됩니다.
 */
...
export async function GET(request: NextRequest) {
  ...
  // ── JSZip 으로 스타일 주입 (런타임 require — 없어도 다운로드는 정상 동작) ──
  let finalBuf: Buffer = outputBuf;
  try {
    // serverExternalPackages: ["jszip"] 덕분에 webpack이 번들링하지 않음
    ...
    for (const xmlPath of ["xl/styles.xml", "xl/theme/theme1.xml"]) {
      const f = tplZip.file(xmlPath);
      if (f) outZip.file(xmlPath, await f.async("uint8array"));
    }

    finalBuf = await outZip.generateAsync({
      type: "nodebuffer", compression: "DEFLATE",
      compressionOptions: { level: 6 },
    }) as Buffer;
  } catch {
    // jszip 미설치 또는 실패 시 서식 없는 버전으로 폴백 (다운로드는 정상)
  }
