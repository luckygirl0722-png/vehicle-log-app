...
// Realtime 때문에 dynamic rendering 강제
...
export default async function AdminDashboardPage() {
  ...
  const [
    ...
    supabase.from("trip_logs")
      ...
      .gte("departure_time", curMonthStart)
      .lt("departure_time",  curMonthEnd),

    // 지난 달 기록
    supabase.from("trip_logs")
      .select("distance_km, toll_fee")
      .gte("departure_time", prevMonthStart)
      .lt("departure_time",  prevMonthEnd),

    // 최근 6개월 월별 집계용
    supabase.from("trip_logs")
      .select("departure_time, distance_km, toll_fee")
      .gte("departure_time", sixMonthsAgo)
      .not("arrival_time", "is", null),

    // 차량별 집계용 (이번 달)
    supabase.from("trip_logs")
      .select("vehicle_id, distance_km, vehicles(plate_number)")
      .gte("departure_time", curMonthStart)
      .lt("departure_time",  curMonthEnd)
  ...
  const currentMonth = {
    ...
    totalDistance:  curRows?.reduce((s, t) => s + (t.distance_km ?? 0), 0) ?? 0,
    totalToll:      curRows?.reduce((s, t) => s + (t.toll_fee ?? 0), 0) ?? 0,
  ...
  for (let i = 5; i >= 0; i--) {
    ...
    monthlyMap[k] = { distance: 0, toll: 0 };
  }
  ...
  vehicleRows?.forEach(row => {
    ...
    const plate = (row.vehicles as any)?.plate_number ?? "미확인";
